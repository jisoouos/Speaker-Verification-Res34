...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=80,inplane=128,embedding_size=192, zero_init_residual=False,
        ...
        self.fc = nn.Linear(5120 * block.expansion, embedding_size)
    ...
    def _forward_impl(self, x):
        ...
        input_x=x
        weight = self.attention(input_x)
        #x = x * weight
        mu = torch.sum(x * weight, dim=3)
        sg = torch.sqrt( ( torch.sum((x**2) * weight, dim=3) - mu**2 ).clamp(min=1e-4) )
